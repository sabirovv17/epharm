...
test('demo session is signed and tampering is rejected', () => {
  ...
  const tamperedSuffix = session.endsWith('x') ? 'y' : 'x'

  assert.match(sessionId ?? '', /^[0-9a-f-]{36}$/i)
  assert.equal(readDemoSession(`${session.slice(0, -1)}${tamperedSuffix}`, SECRET), null)
...
test('demo checkout requires a matching signed session, not metadata alone', () => {
  ...
  assert.equal(
    matchesDemoCustomer({ ...customer, metadata: { demo_customer: true } }, signedSession, SECRET),
    false,
  )
})
