...
> **2026-06-09 — текст «Как работает сверка» приведён к актуальной модели** (i18n ru+kk):
> источники истины = лог Стандарт-Н + Excel-выгрузка + ручная модерация; OCR/ОФД —
> вспомогательно/позже. Логика бэка (`decideFromSources`) уже была такой, устарел только текст.
...
- Поля: товар/акция (имя из каталога по SKU — клиентский `buildProductIndex(useProducts())`, без
  правок бэка), распознанная/ожидаемая сумма, бонус, начислено, OCR-уверенность, кассир, дата,
...
- **Заметка:** мультивыбор «акций» из мобилки (ReceiptDraft.promos) на бэк НЕ передаётся (шлётся
  только фото+QR), поэтому в drawer показывается товар по матчу pending-бонуса/OCR, а не заявленный
