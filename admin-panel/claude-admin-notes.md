...
## 2026-06-22 — Промо: переключатель режима просмотра (сетка ↔ список)

По просьбе пользователя добавлен переключатель вида в разделе «Промо-кампании» (рядом
с «Новая кампания», слева от «Обновить цены»). Два режима:

- **Сетка** (дефолт): прежние карточки, но превью кампании = **фото товара из Medusa в
  формате 3:4** (`aspect-[3/4]`, `object-cover`), через `proxyMedia` (http→HTTPS-прокси,
  как в детали/галерее). Нет фото → фирменный градиент-заглушка (`promo.cover`). Поверх
  фото — скрим снизу + статус-чип + бренд/название.
- **Список**: таблица как реестр контрактов Ledex — мини-превью 3:4 (36×48) + название/бренд,
  статус, период, кол-во аптек, бюджет (spent/budget + ProgressBar), действия
  (пауза/архив или восстановить). Строка кликабельна → `/promo/:id`; inline-кнопки
  `stopPropagation`.

Реализация: `PromoPage.tsx` — `ViewMode='grid'|'list'`, выбор персистится в
`localStorage['epharm.promoView']` (через try/catch — приватный режим не ломает).
Эффективное фото: `promoPhoto()` = `overrideImage ?? productImage` → `proxyMedia` либо null.
Новые компоненты: `ViewModeToggle`, `PromoListView`, `PromoRow`. Иконки `IconGrid`/`IconList`
добавлены в `ui/icons.tsx`. i18n-ключи `pm.view*`/`pm.col*` (ru+kk).

Тест-нюанс: в этой jsdom `localStorage` глобально недоступен (как в store.test.ts) — в
