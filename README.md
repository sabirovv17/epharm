...
| **Backend** (`backend/`)                   | Spring Boot монолит — REST для mobile + admin + POSM, JWT, Flyway, S3                                                                              | Kotlin 2.0 + Spring Boot 3.3 + PostgreSQL 16 + Redis 7 + MinIO | Этап 0 — `/api/health` + миграция V001                 |
