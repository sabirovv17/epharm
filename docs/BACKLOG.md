# Бэклог

Актуальный список технических рисков и работ до полноценного промышленного запуска.
Обновлено: 2026-08-04.

## Текущее состояние

| Слой                           | Проверка                                                 | Production                                                                 |
| ------------------------------ | -------------------------------------------------------- | -------------------------------------------------------------------------- |
| Backend (Kotlin)               | 368/368 тестов                                           | healthy, Flyway v036                                                       |
| Админ-фронт (React)            | 378/378 тестов, production build успешен                 | обучение и эфир до 12 роликов развёрнуты                                   |
| Мобильное приложение (Flutter) | analyze: 0 issues, 107/107 тестов                        | iOS 0.1.2+3 собран; установка ждёт свежий provisioning profile             |
| POSM (C#)                      | интеграционные сценарии покрыты backend/frontend тестами | четыре кассы online; одинаковые Windows deviceId больше не затирают аптеки |
| OTP                            | фиксированный код `544544` проверен по production API    | P1SMS намеренно отключён, SMS не отправляются                              |

Существующие production-данные после релиза сохранены: кампании, правила, плейлисты,
слайды и администратор не изменены. На 2026-08-04 в базе ещё нет фармацевтов,
учебных программ и назначений; это ожидает бизнес-настройки через админку.

## P0 — до подключения реальных пользователей

- [ ] **Вернуть безопасный OTP.** Текущий `OTP_DEV_MODE=true` и общий код `544544` подходят
      только для закрытого пилота. Перед публичным запуском подключить SMS-провайдера или другой
      проверенный фактор, выключить dev-mode и убедиться, что `devCode` не возвращается клиенту.
- [ ] **Настроить Apple signing.** Добавить Apple Developer account в Xcode, выпустить свежий
      provisioning profile для `kz.pharmacy.app`, установить сборку на iPhone. Для тиражирования
      использовать TestFlight/MDM, а не недельные free-development profiles.
- [ ] **Заменить недоступный Medusa origin.** Production всё ещё ссылается на
      `http://78.140.246.238:9000`; старый сервер выведен из эксплуатации. Нужен актуальный URL,
      ключи и smoke-тест каталога, иначе холодные запросы каталога будут зависать/падать.
- [ ] **Закрыть frontend dependency vulnerabilities.** `npm audit` показывает 8 пакетов:
      6 high и 2 moderate (`axios`, `postcss`, `vite`, `form-data`, `undici`, `react-router` и
      транзитивные зависимости). Обновлять с regression-тестами, особенно React Router major.
- [ ] **Разблокировать GitHub Actions.** Все jobs PR сейчас не стартуют из-за billing lock
      аккаунта GitHub. После разблокировки сделать checks обязательными перед merge в `main`.

## P1 — эксплуатационная готовность

- [ ] **Довести frontend lint до нуля.** Сейчас 19 errors и 1 warning: Playwright fixture
      false-positive, Fast Refresh exports и синхронный `setState` в effects. Тесты и build зелёные,
      но lint не должен оставаться красным.
- [ ] **Обновить Node runtime сборки.** Текущий frontend image использует Node 20, тогда как
      одна из зависимостей объявляет engine Node 22.22.2+. Зафиксировать поддерживаемую LTS-версию
      и пересобрать lockfile/image.
- [ ] **Провести production acceptance обучения.** Создать активного фармацевта, программу,
      этапы и назначение; пройти маршрут в мобильном приложении, тест, награду, сертификат,
      повторное назначение и проверку RBAC каждой административной роли.
- [ ] **Настроить сопоставление Standard-N продавцов.** В production приходят внешние USER_ID
      (например `33`), но внутренних фармацевтов пока нет. Создать профили и явное mapping-правило,
      иначе продажа видна, но бонус нельзя безопасно закрепить за человеком.
- [ ] **Провести POSM acceptance на реальной кассе.** Проверить рекомендацию, удаление триггера,
      чек на клиентском экране, 12-роликовый эфир, offline-outbox, reboot/watchdog и presence.
- [ ] **Выдавать отдельный POSM key на устройство.** Общий `POSM_DEVICE_KEY` не подходит для
      500 касс: нужна выдача/ротация/отзыв ключей и аудит устройства.
- [ ] **Исправить обработку неизвестных API-маршрутов.** `NoResourceFoundException` сейчас
      попадает в generic handler и возвращает 500 вместо 404, создавая ложные production errors.
- [ ] **Убрать шум Spring Data Redis/JPA при старте.** Явно разделить repository scan packages;
      сейчас каждый JPA repository проверяется как кандидат Redis.

## P2 — надёжность и сопровождение

- [ ] Автоматизировать PostgreSQL и MinIO backup, retention и регулярный restore-test.
- [ ] Добавить uptime, error-rate, latency, disk/DB/MinIO monitoring и оповещения.
- [ ] Подключить Sentry или аналог для backend, admin frontend и мобильного приложения.
- [ ] Ввести immutable release id/tag, changelog и проверяемый rollback для каждого деплоя.
- [ ] Провести нагрузочный тест сценариев 500 касс: heartbeat, playlist polling, offline sync,
      продажи, рекомендации и массовая загрузка роликов.
- [ ] Завершить mobile release checklist: TestFlight, privacy manifests, QR/камера на реальном
      устройстве, deep links, восстановление сессии и обработка недоступного fallback `:8060`.

## Выполнено 2026-08-04

- [x] Production backup БД, исходников, конфигурации и image ids перед релизом.
- [x] Миграции обучения v035/v036 применены без потери существующих данных.
- [x] Админка, API обучения, LMS, эфир до 12 роликов и оба ingress-адреса отвечают 200.
- [x] P1SMS отключён, фиксированный OTP `544544` проверен request/verify smoke-тестом.
- [x] Presence хранится по `(pharmacyId, deviceId)`: четыре кассы видны одновременно.
- [x] Полные backend, frontend и Flutter test suites проходят локально.
