...
- [ ] **Провести fulfillment-пилот на двух кассах одной аптеки.** До включения связать аптеку
      ...
      включение до решения и пилота запрещено. Для Абая 150/230 (Алматы) точный mapping
      `sloc_01KSAHYDSFE9QRRNK42YZB1D2S` → `ch:432`, ACC worker и подписанный status feed
      проверены; на 2026-09-22 касса не опрашивает backend с 18:55 UTC. Таймер выдачи намеренно
      выключен до восстановления heartbeat, подтверждения окна POSM и согласованного заказа.
...
- [ ] **Принять реальный ACC order bridge и расширить пилот.** Отдельный worker развернут с
      ...
      offline/retry и полную смену без ошибок. Только затем поэтапно расширять список аптек.
