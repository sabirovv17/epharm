...
ApiAuthRepository _repo(TokenStore store, MockClient client) =>
    ApiAuthRepository(
        ApiClient(store, baseUrl: 'http://t', client: client), store);
...
void main() {
  ...
  test('verifyOtp registered=true → сохраняет токены и маппит профиль',
      () async {
    final store = TokenStore();
    final repo = _repo(
        store,
  ...
  test('register → токены + pending-профиль с нулевым балансом', () async {
    ...
    final u = await repo.register(
        phone: '+77771002030', fio: 'Тест Тестов', iin: '990101777777');
