...
dotnet publish App\CustomerDisplay.csproj -c Release -r win-x64 --self-contained `
  -p:Version=1.0.64 -o C:\Epharm\app
```
...
4. Sign the exact manifest with the offline ECDSA P-256 private key:
   ...
   ```bash
   tools/sign-posm-release.sh private-key.pem win-x64 1.0.64 \
     https://epharm.inkar.kz/downloads/epharm-posm-1.0.64-win-x64.zip release.zip false
   ```
