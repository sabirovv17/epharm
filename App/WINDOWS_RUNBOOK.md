...
3. Запусти приложение и допиши в лог строку `Add2Cheque ... iPartID=80309(...) ...` (см. п.5) —
...
> В синтетическом примере `iPartID=80309(80309)` штрих-кода нет → уходит только `name` (этого
> достаточно для демо). Когда касса начнёт писать реальный EAN — матчинг по `barcode` заработает
> сам, без изменений клиента.
